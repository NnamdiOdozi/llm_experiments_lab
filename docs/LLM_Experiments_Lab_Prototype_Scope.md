...
| **Training execution** | In-process: a background thread in the same Python process runs the PyTorch training loop directly on your CPU | Out-of-process: dispatched as a **Nebius Serverless Job** running inside a **Docker container** | Unchanged from Tier 2 |
| **Containerisation** | ❌ None — training code imported and called directly | ✅ Dockerfile for the training image; build + push to a registry | ✅ Unchanged |
...
| **Pause-and-prompt** | Trivial: model stays in memory in the same process; "pause" just stops the loop and accepts an inference call directly | Real implementation: checkpoint → Job exits → spin up a **Nebius Serverless Endpoint** on that checkpoint → inference → tear down → new Job resumes | ✅ Unchanged |
...
2. **Pause-and-prompt is a fundamentally different implementation in Tier 1 vs. Tiers 2/3.** Tier 1: the model object stays in memory in the same Python process — "pause" is just stopping a loop and calling `model.generate(...)` directly. Tiers 2/3: it's *checkpoint, kill the Job, stand up a separate Endpoint, tear it down, start a new Job to resume* — four moving parts instead of zero. Don't try to write Tier 1's pause-and-prompt code to be "reusable later" — it largely isn't.
